...
(function (y, w, z) {
    ...
    r.directive("ngClick", ["$parse", "$timeout", "$rootElement", function (f, a, c) {
        ...
        function p(a) {
            if (!(Date.now() - n > g)) {
                var b =
                        a.touches && a.touches.length ? a.touches : [a], c = b[0].clientX, b = b[0].clientY;
        ...
        return function (a, b, e) {
            ...
            b.on("touchend", function (a) {
                var g = Date.now() - r, f = a.changedTouches && a.changedTouches.length ? a.changedTouches : a.touches && a.touches.length ?
                    a.touches : [a], h = f[0].originalEvent || f[0], f = h.clientX, h = h.clientY, s = Math.sqrt(Math.pow(f - u, 2) + Math.pow(h - x, 2));
